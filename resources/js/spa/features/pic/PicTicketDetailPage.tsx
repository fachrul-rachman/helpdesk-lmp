...
export function PicTicketDetailPage() {
    ...
    const takeoverState = useMemo(() => {
        ...
        if (!req)
            return {
                status: null as null | 'pending' | 'approved',
                isRequester: false,
            };
        const isRequester = !!(
            currentUser?.id && req.requested_by?.id === currentUser.id
        );
        return { status: req.status, isRequester };
    }, [ticket, currentUser]);

    const isReadOnly =
        takeoverState.status === 'approved' && takeoverState.isRequester;
    ...
    async function handleRequestTakeover() {
        ...
        setSendError(null);
        try {
            await api.post(`/api/pic/tickets/${id}/takeover-request`, {
            ...
            const message =
                error?.response?.data?.message ??
                'Gagal mengirim request takeover.';
    ...
    async function handleCancelTakeover() {
        ...
        try {
            ...
            const message =
                error?.response?.data?.message ??
                'Gagal membatalkan request takeover.';
            setSendError(String(message));
        } finally {
            setIsTakeoverSubmitting(false);
        }
    ...
    async function handleSaveNotes() {
        ...
        try {
            ...
            if (ticket.customer?.id) {
                ops.push(
                    api.patch<{ data: any }>(
                        `/api/customers/${ticket.customer.id}/notes`,
                        {
            ...
            const updatedTicket = results
                ...
                .find(
                    (d: any) => d && typeof d === 'object' && 'subject' in d,
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_380px] lg:items-start">
                ...
                <div className="hidden space-y-4 lg:order-2 lg:block lg:max-h-[calc(100dvh-140px)] lg:overflow-auto lg:pr-1">
                    <CollapsibleCard title="Data Customer">
                        ...
                        <div className="mt-4">
                            ...
                            <Textarea
                                ...
                                onChange={(e) =>
                                    setCustomerNotesDraft(e.target.value)
                                }
                    ...
                    <CollapsibleCard title="Data Ticket" defaultOpen>
                        ...
                        <div className="mt-5 border-t border-slate-200 pt-4 text-xs font-semibold tracking-wide text-slate-500 uppercase">
                            Penanganan
                        </div>
                    ...
                    {['open', 'pending', 'on_progress'].includes(
                        ticket.status,
                    ) ? (
                        <div className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
                            ...
                            {ticket.takeover_request ? (
                                    ...
                                    takeoverState.isRequester ? (
                                      ...
                                      takeoverState.isRequester ? (
                                        <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-2 text-sm text-amber-800">
                                            Ticket sedang diambil alih oleh SPV.
                                            Anda tidak bisa melakukan aksi
                                            sampai request ditutup.
                                        </div>
                                    ) : null}
                                </>
                            ) : (
                                <>
                                    <div className="mt-2 text-xs text-slate-600">
                                        Ajukan request takeover ke SPV jika Anda
                                        butuh bantuan atau sedang ada pekerjaan
                                        lain.
                                    </div>
                                    <Textarea
                                        rows={3}
                                        ...
                                        onChange={(e) =>
                                            setTakeoverReason(e.target.value)
                                        }
                                        className="mt-2"
                                        disabled={
                                            isTakeoverSubmitting || isReadOnly
                                        }
                                    />
                                    <Button
                                        className="mt-3 w-full"
                    ...
                    <div className="absolute inset-x-0 bottom-0 max-h-[85dvh] overflow-auto rounded-t-2xl bg-slate-50 p-4 shadow-xl">
                        ...
                        <div className="space-y-4">
                            ...
                            <Button
                                className="sticky bottom-0 w-full"
                                onClick={handleSaveNotes}
                                disabled={isSavingNotes || isReadOnly}
                            >
                                {isSavingNotes ? 'Memperbarui...' : 'Update'}
                            </Button>
                        </div>

...
export function SpvTicketDetailPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                ...
                if (!isMounted) return;
                const message =
                    error?.response?.data?.message ?? 'Gagal memuat detail ticket.';
    ...
    const canReply = useMemo(() => {
        ...
        if (ticket.status === 'solved' || ticket.status === 'closed') return false;
    ...
    async function handleApproveTakeover() {
        ...
        try {
            ...
            const message = error?.response?.data?.message ?? 'Gagal approve request takeover.';
            setErrorMessage(String(message));
        } finally {
    ...
    async function handleRejectTakeover() {
        ...
        try {
            ...
            const message = error?.response?.data?.message ?? 'Gagal reject request takeover.';
            setErrorMessage(String(message));
        } finally {
            setIsHandlingTakeover(false);
        }
    ...
    async function handleCloseTakeover() {
        ...
        try {
            ...
            const message = error?.response?.data?.message ?? 'Gagal close request takeover.';
            setErrorMessage(String(message));
        } finally {
            setIsHandlingTakeover(false);
        }
    ...
    async function handleUpdate() {
        ...
        try {
            ...
            if (nextPicId && !nextDivisionId) {
                const res = await api.patch<TicketDetailResponse>(`/api/tickets/${ticket.id}/assign`, {
                    user_id: nextPicId,
                });
                setTicket(res.data.data);
            }
            ...
            if (nextPriority) {
                const res = await api.patch<TicketDetailResponse>(`/api/tickets/${ticket.id}/priority`, {
            ...
            const message = error?.response?.data?.message ?? 'Gagal memperbarui ticket.';
            setErrorMessage(String(message));
        } finally {
            setIsUpdating(false);
        }
    ...
    if (isLoading) return <div className="text-sm text-slate-600">Memuat ticket...</div>;
    if (errorMessage)
        return (
    ...
    const DetailPanel = (
        <div className="space-y-4">
            ...
            <div className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
                    ...
                    <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                ...
                                <div className="mt-1 text-xs text-slate-600">
                                    Dari: {ticket.takeover_request.requested_by?.name ?? '-'} ({ticket.takeover_request.status})
                                </div>
                            </div>
                            <div className="shrink-0">
                                ...
                                >
                                    {ticket.takeover_request.status === 'approved' ? 'Approved' : 'Pending'}
                                </span>
                            </div>
                        </div>

                        <div className="mt-2 whitespace-pre-wrap text-sm text-slate-800">{ticket.takeover_request.reason}</div>

                        <div className="mt-3 flex flex-wrap gap-2">
                            {ticket.takeover_request.status === 'pending' ? (
                                <>
                                    <Button onClick={handleApproveTakeover} disabled={isHandlingTakeover}>
                                        {isHandlingTakeover ? 'Memproses...' : 'Approve'}
                                    </Button>
                                    <Button variant="secondary" onClick={handleRejectTakeover} disabled={isHandlingTakeover}>
                                        Reject
                                    </Button>
                                </>
                            ) : (
                                <Button variant="secondary" onClick={handleCloseTakeover} disabled={isHandlingTakeover}>
                                    {isHandlingTakeover ? 'Memproses...' : 'Close Request'}
                                </Button>
                            )}
                        </div>
                    </div>
                ...
                <div className="mt-4 space-y-2">
                    <div className="text-xs font-semibold text-slate-700">Ubah Prioritas</div>
                    <select
                        className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        value={nextPriority}
                        onChange={(e) => setNextPriority((e.target.value as any) || '')}
                        disabled={isUpdating}
                    >
                ...
                <div className="mt-4 space-y-2">
                    <div className="text-xs font-semibold text-slate-700">Pindah Divisi</div>
                    <select
                        className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="mt-4 space-y-2">
                    <div className="text-xs font-semibold text-slate-700">Assign PIC</div>
                    <select
                        className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        value={nextPicId}
                    ...
                    {!nextDivisionId && ticket.division?.id ? (
                        <div className="text-xs text-slate-600">
                            Untuk assign cepat, pilih divisi di atas (atau gunakan tombol “Update” setelah pilih PIC).
                        </div>
                    ) : null}
                </div>
                        ...
                        <div className="border-t border-slate-200 p-3">
                            {!canReply ? (
                                <div className="mb-2 rounded-md border border-amber-200 bg-amber-50 p-2 text-sm text-amber-800">
                                    Anda hanya bisa membalas ticket fallback (assigned ke SPV).
                                </div>
                            ) : null}
                            {sendError ? (
